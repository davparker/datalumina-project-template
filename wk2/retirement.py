...
def displayRetirementWithRates(monthly, rates, terms):
    ...
    for rate in rates:
        xvals, yvals = retire(monthly, rate, terms)
        plt.plot(
            xvals, yvals, label="retire:" + str(monthly) + ":" + str(int(rate * 100))
        )
        plt.legend(loc="upper left")


displayRetirementWithRates(800, [0.03, 0.05, 0.07], 40 * 12)
...
displayRetirementWithMonthsAndRates([500, 700, 900, 1100], [0.03, 0.05, 0.07], 40 * 12)
